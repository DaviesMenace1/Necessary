...
function ProductDialog({ item, onClose }: { item: CatalogItem | null; onClose: () => void }) {
...
function buildSpecs(item: CatalogItem): Array<[string, string]> {
  return [
...
function originFor(m: CatalogItem["manufacturer"]): string {
  switch (m) {
    case "Boca do Lobo":
