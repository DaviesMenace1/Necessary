...
function ArchitecturePage() {
  const { image } = usePageContent("architecture");
  const heroImg = image("hero", 0, archVilla);
  const showcaseMain = image("showcase", 0, archInterior);
  const showcasePlan = image("showcase", 1, archPlan);
  const showcaseSketch = image("showcase", 2, archSketch);
  return (
    <>
      {/* Hero */}
